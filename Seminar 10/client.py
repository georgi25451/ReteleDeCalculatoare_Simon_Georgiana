...
class FTPClient:
    ...
    def choose_server_file(self):
        """Ask the user to choose a file from the server by number or name."""
    ...
    def download(self):
        ...
        local_filename = response.get('filename', filename)
        local_path = os.path.join(LOCAL_FILES_DIR, local_filename)

        try:
            with open(local_path, 'w', encoding='utf-8') as f:
                f.write(content)
            print(f"✓ {response['message']}")
            print(f"✓ File saved locally in {LOCAL_FILES_DIR}/{local_filename}")
        except Exception as e:
