...
while True:
    ...
    if comanda == 'EXIT':
        ...
        if not este_conectat:
            print("EROARE: Trebuie sa fii conectat pentru a publica mesaje.")
            continue
        if len(parti) < 2 or not parti[1].strip():
            print("EROARE: Comanda PUBLISH necesita un text (exemplu: PUBLISH Salut).")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
        ...
        if not este_conectat:
            print("EROARE: Trebuie sa fii conectat pentru a sterge mesaje.")
            continue
        if len(parti) < 2 or not parti[1].isdigit():
            print("EROARE: Comanda DELETE necesita un ID numeric (exemplu: DELETE 1).")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
        ...
        if not este_conectat:
            print("EROARE: Trebuie sa fii conectat pentru a lista mesaje.")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
